...
struct Token
{
    ...
    string Value;
    bool IsString;
    Token(string value = "", TokenType type = TokenType::Argument) : Value(value), Type(type), IsString(false) {}
};
...
class Tokenizer
{
  ...
  public:
    static vector<Token> Tokenize(const string &s, bool testing = false)
    {
        vector<Token> ret;
        ...
        while (ss >> token)
        {
            ...
            if(isQuoted || openString) {
                ...
                if(testing) cout << str << " " << isQuoted << " " << openString << " _" << thisQuote << "_ _" << quoteUsed << "_" << endl;
